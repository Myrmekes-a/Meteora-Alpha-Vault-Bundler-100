...
import { DEVNET_USDC_MINT, MAINNET_USDC_MINT, POOL_ACTIVATION_FROM_NOW_SEC } from "../lib/constants";
...
async function getTokenMintFromOutputFile(pathValue: string): Promise<PublicKey> {
  ...
  if (!parsed) {
    throw new Error(`token mint output not found for key: ${pathValue}`);
  }

  if (!parsed.tokenMint) {
    throw new Error(`tokenMint missing in output for key: ${pathValue}`);
  }
...
async function writePoolOutput(params: {
  ...
  await saveArtifactByKey("pool-output", params.outputPath, {
...
async function main(): Promise<void> {
  ...
  if (connectAlphaVaultPool) {
    ...
    const raw = process.env.POOL_ACTIVATION_POINT_TS?.trim();
    let activationValue: number;
    if (raw) {
      const parsed = parseInt(raw, 10);
      if (Number.isNaN(parsed)) {
        throw new Error(`POOL_ACTIVATION_POINT_TS must be a number, got: ${raw}`);
      }
      // If value < 2 years in seconds, treat as "seconds from now"
      const TWO_YEARS_SEC = 86400 * 365 * 2;
      activationValue = parsed < TWO_YEARS_SEC ? now + parsed : parsed;
    } else {
      activationValue = now + POOL_ACTIVATION_FROM_NOW_SEC;
    }
    const activationPointTs = new BN(String(activationValue), 10);
    console.log("🚀 ~ main ~ activationPointTs:", activationPointTs.toNumber())
